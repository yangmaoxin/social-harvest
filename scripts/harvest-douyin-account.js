...
export function printHelp() {
  console.log(`
Usage:
  node scripts/harvest-douyin-account.js --date <YYYY-MM-DD> [options]
...
export async function runDouyinCreatorAccount(options) {
  ...
  const report = {
    ...
    warnings: rows[0]?.account_id ? [] : ['No account_id extracted from douyin creator account APIs.'],
  };
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const result = await runDouyinCreatorAccount(options);
